...
description: Onboarding Session 06 (Taxonomy). Builds the foundation taxonomy profile via the structured reflective listening loop; runs discovery, a required-fields pass, and a closing checkpoint; writes captured_items rows and renders foundation/06-taxonomy.yaml at session close. Final onboarding session.
category: onboarding
phase: interview
version: 0.2.0
agent: hermes
trigger: ["/solomon-onboarding-06-taxonomy", "start session 06", "begin taxonomy session"]
inputs:
  - active_domain (set to "taxonomy")
  - probe_library/taxonomy.yaml (keywords, fallbacks, required_fields, probe_style)
  - db.sessions (open or resume the row for this session)
  - db.coverage (rows where domain="taxonomy")
...
outputs:
  ...
  - foundation/06-taxonomy.yaml (rendered by query F3 at session close)
reads_only: false
autonomy_level: L1
depends_on:
  - solomon-redact
...
# solomon-onboarding-06-taxonomy

Session 06 of the 7-session onboarding flow, and the final one. Builds the foundation taxonomy profile (operational scopes, decision-type labels, customer segments, product/service buckets, vendor groups, revenue streams, internal jargon) into `db.captured_items`. Compiles `foundation/06-taxonomy.yaml` at session close. The captured taxonomy seeds `scope_autonomy` (per SOLOMON-PLAN.md §2.7), the corpus `entity_allowlist`, and downstream classifiers.
...
`taxonomy`. The interview-engine reads `probe_library/taxonomy.yaml`, which holds keywords, fallbacks, the seven required_fields, and the probe_style block.

## Listening style

All discovery probes and required-field follow-ups follow the seven rules in `probe_library/taxonomy.yaml::probe_style` and `references/eliza-listening.md`. Summary: use the owner's exact words, do not editorialize, build follow-ups on the echoed phrase, drop filler, keep it short, follow emotional content, pivot plainly when the topic shifts.

## Five-stage flow

### Stage A. Setup

1. Compute `session_id`. Format: `onboarding-06-YYYY-MM-DD`. If a row already exists for the date, append `-N` (where N is the next available integer for the day).
...
3. If no resumable row exists, INSERT into `db.sessions` with `type='onboarding'`, `domain='taxonomy'`, `status='active'`, `started_at=NOW()`, `last_activity_at=NOW()`, `turns=0`.
4. Set `active_domain = "taxonomy"`.
5. Run the probe-library version migration check (per `solomon-coverage-tracker`): if `coverage.library_version_seen < probe_library/taxonomy.yaml::version` for `domain='taxonomy'`, write a `mentoring_queue` row (`source='probe_library_update'`, `priority=7`).
...
2. For each unfilled `field_id` in declaration order:
   a. Ask the field's `prompt` verbatim from `taxonomy.yaml::required_fields`.
...
2. Run query F3 to render `foundation/06-taxonomy.yaml`. The file format follows the standard header convention used by `foundation/05-non-negotiables.yaml`: a leading comment block, `last_updated`, then a `required_fields` map (one entry per field id with `statement` and `verbatim_phrase`), then a `discovery` map keyed by sub-topic, then a `voice_samples` list (top vocabulary phrases by frequency from this session). DB stays canonical per SOLOMON-PLAN.md §1; the YAML is a derived summary.
3. Print: "Session 06 is complete. All 7 onboarding sessions are done. Solomon's foundation is set."
...
### F3. Render foundation/06-taxonomy.yaml at session close
