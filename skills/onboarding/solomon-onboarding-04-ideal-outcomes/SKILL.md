...
description: Onboarding Session 04 (Ideal Outcomes). Builds the foundation ideal-outcomes profile via the structured reflective listening loop; runs discovery, a required-fields pass, and a closing checkpoint; writes captured_items rows and renders foundation/04-ideal-outcomes.yaml at session close.
category: onboarding
phase: interview
version: 0.2.0
agent: hermes
trigger: ["/solomon-onboarding-04-ideal-outcomes", "start session 04", "begin ideal outcomes session"]
inputs:
  - active_domain (set to "ideal-outcomes")
  - probe_library/ideal-outcomes.yaml (keywords, fallbacks, required_fields, probe_style)
  - db.sessions (open or resume the row for this session)
  - db.coverage (rows where domain="ideal-outcomes")
...
outputs:
  ...
  - foundation/04-ideal-outcomes.yaml (rendered by query F3 at session close)
reads_only: false
autonomy_level: L1
depends_on:
  - solomon-redact
...
# solomon-onboarding-04-ideal-outcomes

Session 04 of the 7-session onboarding flow. Builds the foundation ideal-outcomes profile (3-year revenue, team size, owner workload, customer mix, geographic footprint, one-year milestone, five-year vision) into `db.captured_items`. Compiles `foundation/04-ideal-outcomes.yaml` at session close.
...
`ideal-outcomes`. The interview-engine reads `probe_library/ideal-outcomes.yaml`, which holds keywords, fallbacks, the seven required_fields, and the probe_style block.

## Listening style

All discovery probes and required-field follow-ups follow the seven rules in `probe_library/ideal-outcomes.yaml::probe_style` and `references/eliza-listening.md`. Summary: use the owner's exact words, do not editorialize, build follow-ups on the echoed phrase, drop filler, keep it short, follow emotional content, pivot plainly when the topic shifts.

## Five-stage flow

### Stage A. Setup

1. Compute `session_id`. Format: `onboarding-04-YYYY-MM-DD`. If a row already exists for the date, append `-N` (where N is the next available integer for the day).
2. Read `db.sessions` for any row matching `session_id LIKE 'onboarding-04-%' AND status='active'`. If one exists, resume that `session_id` instead of opening a new row.
3. If no resumable row exists, INSERT into `db.sessions` with `type='onboarding'`, `domain='ideal-outcomes'`, `status='active'`, `started_at=NOW()`, `last_activity_at=NOW()`, `turns=0`.
4. Set `active_domain = "ideal-outcomes"`.
5. Run the probe-library version migration check (per `solomon-coverage-tracker`): if `coverage.library_version_seen < probe_library/ideal-outcomes.yaml::version` for `domain='ideal-outcomes'`, write a `mentoring_queue` row (`source='probe_library_update'`, `priority=7`).
...
2. For each unfilled `field_id` in declaration order:
   a. Ask the field's `prompt` verbatim from `ideal-outcomes.yaml::required_fields`.
...
### F3. Render foundation/04-ideal-outcomes.yaml at session close
