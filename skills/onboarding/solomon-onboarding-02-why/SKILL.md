---
name: solomon-onboarding-02-why
description: Onboarding Session 02 (Why). Builds the foundation why profile via the structured reflective listening loop; runs discovery, a required-fields pass, and a closing checkpoint; writes captured_items rows and renders foundation/02-why.yaml at session close.
category: onboarding
phase: interview
version: 0.2.0
agent: hermes
trigger: ["/solomon-onboarding-02-why", "start session 02", "begin why session"]
inputs:
  - active_domain (set to "why")
  - probe_library/why.yaml (keywords, fallbacks, required_fields, probe_style)
  - db.sessions (open or resume the row for this session)
  - db.coverage (rows where domain="why")
...
outputs:
  ...
  - foundation/02-why.yaml (rendered by query F3 at session close)
reads_only: false
autonomy_level: L1
depends_on:
  - solomon-redact
...
# solomon-onboarding-02-why

Session 02 of the 7-session onboarding flow. Builds the foundation why profile (origin story, personal motivation, mission, exit horizon, definition of success, quit conditions, why this business specifically) into `db.captured_items`. Compiles `foundation/02-why.yaml` at session close.
...
`why`. The interview-engine reads `probe_library/why.yaml`, which holds keywords, fallbacks, the seven required_fields, and the probe_style block.

## Listening style

All discovery probes and required-field follow-ups follow the seven rules in `probe_library/why.yaml::probe_style` and `references/eliza-listening.md`. Summary: use the owner's exact words, do not editorialize, build follow-ups on the echoed phrase, drop filler, keep it short, follow emotional content, pivot plainly when the topic shifts.

## Five-stage flow

### Stage A. Setup

1. Compute `session_id`. Format: `onboarding-02-YYYY-MM-DD`. If a row already exists for the date, append `-N` (where N is the next available integer for the day).
2. Read `db.sessions` for any row matching `session_id LIKE 'onboarding-02-%' AND status='active'`. If one exists, resume that `session_id` instead of opening a new row.
3. If no resumable row exists, INSERT into `db.sessions` with `type='onboarding'`, `domain='why'`, `status='active'`, `started_at=NOW()`, `last_activity_at=NOW()`, `turns=0`.
4. Set `active_domain = "why"`.
5. Run the probe-library version migration check (per `solomon-coverage-tracker`): if `coverage.library_version_seen < probe_library/why.yaml::version` for `domain='why'`, write a `mentoring_queue` row (`source='probe_library_update'`, `priority=7`).
...
2. For each unfilled `field_id` in declaration order:
   a. Ask the field's `prompt` verbatim from `why.yaml::required_fields`.
...
### F3. Render foundation/02-why.yaml at session close
