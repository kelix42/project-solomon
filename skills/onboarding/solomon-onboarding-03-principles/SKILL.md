...
description: Onboarding Session 03 (Principles). Builds the foundation principles profile via the structured reflective listening loop; runs discovery, a required-fields pass, and a closing checkpoint; writes captured_items rows and renders foundation/03-principles.yaml at session close.
category: onboarding
phase: interview
version: 0.2.0
agent: hermes
trigger: ["/solomon-onboarding-03-principles", "start session 03", "begin principles session"]
inputs:
  - active_domain (set to "principles")
  - probe_library/principles.yaml (keywords, fallbacks, required_fields, probe_style)
  - db.sessions (open or resume the row for this session)
  - db.coverage (rows where domain="principles")
...
outputs:
  ...
  - foundation/03-principles.yaml (rendered by query F3 at session close)
reads_only: false
autonomy_level: L1
depends_on:
  - solomon-redact
...
# solomon-onboarding-03-principles

Session 03 of the 7-session onboarding flow. Builds the foundation principles profile (pricing, hiring/firing, customer conflict, money, quality vs speed, saying no, growth) into `db.captured_items`. Compiles `foundation/03-principles.yaml` at session close.
...
`principles`. The interview-engine reads `probe_library/principles.yaml`, which holds keywords, fallbacks, the seven required_fields, and the probe_style block.

## Listening style

All discovery probes and required-field follow-ups follow the seven rules in `probe_library/principles.yaml::probe_style` and `references/eliza-listening.md`. Summary: use the owner's exact words, do not editorialize, build follow-ups on the echoed phrase, drop filler, keep it short, follow emotional content, pivot plainly when the topic shifts.

## Five-stage flow

### Stage A. Setup

1. Compute `session_id`. Format: `onboarding-03-YYYY-MM-DD`. If a row already exists for the date, append `-N` (where N is the next available integer for the day).
2. Read `db.sessions` for any row matching `session_id LIKE 'onboarding-03-%' AND status='active'`. If one exists, resume that `session_id` instead of opening a new row.
3. If no resumable row exists, INSERT into `db.sessions` with `type='onboarding'`, `domain='principles'`, `status='active'`, `started_at=NOW()`, `last_activity_at=NOW()`, `turns=0`.
4. Set `active_domain = "principles"`.
5. Run the probe-library version migration check (per `solomon-coverage-tracker`): if `coverage.library_version_seen < probe_library/principles.yaml::version` for `domain='principles'`, write a `mentoring_queue` row (`source='probe_library_update'`, `priority=7`).
...
2. For each unfilled `field_id` in declaration order:
   a. Ask the field's `prompt` verbatim from `principles.yaml::required_fields`.
...
### F3. Render foundation/03-principles.yaml at session close
