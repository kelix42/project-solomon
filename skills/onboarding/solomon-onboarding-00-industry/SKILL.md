...
description: Onboarding Session 0 (Industry). Builds the foundation industry profile via the structured reflective listening loop; runs discovery, a required-fields pass, and a closing checkpoint; writes captured_items rows and renders foundation/00-industry.yaml at session close.
category: onboarding
phase: interview
version: 0.2.0
agent: hermes
trigger: ["/solomon-onboarding-00-industry", "start session 0", "begin industry session"]
inputs:
  - active_domain (set to "industry")
  - probe_library/industry.yaml (keywords, fallbacks, required_fields, probe_style)
  - db.sessions (open or resume the row for this session)
  - db.coverage (rows where domain="industry")
...
outputs:
  ...
  - foundation/00-industry.yaml (rendered by query F3 at session close)
reads_only: false
autonomy_level: L1
depends_on:
  - solomon-redact
...
Session 0 of the 7-session onboarding flow. Builds the foundation industry profile (industry, sector, geography, customers, ecosystem, risks, trends) into `db.captured_items`. Compiles `foundation/00-industry.yaml` at session close.

This skill is a thin wrapper over `solomon-interview-engine` plus the post-turn extraction loop, per §6 of SOLOMON-PLAN.md. It adds two structural elements on top of the spec's discovery saturation: a required-fields pass and a closing checkpoint. The same pattern will be applied to Sessions 1 to 6 in later migration slices.

## Active domain

`industry`. The interview-engine reads `probe_library/industry.yaml`, which holds keywords, fallbacks, the seven `required_fields`, and the `probe_style` block.

## Listening style

All discovery probes and required-field follow-ups follow the seven rules in `probe_library/industry.yaml::probe_style` and `references/eliza-listening.md`. Summary: use the owner's exact words, do not editorialize, build follow-ups on the echoed phrase, drop filler, keep it short, follow emotional content, and pivot plainly when the topic shifts.

## Five-stage flow

### Stage A. Setup

1. Compute `session_id`. Format: `onboarding-00-YYYY-MM-DD`. If a row already exists for the date, append `-N` (where N is the next available integer for the day).
2. Read `db.sessions` for any row matching `session_id LIKE 'onboarding-00-%' AND status='active'`. If one exists, resume that `session_id` instead of opening a new row.
3. If no resumable row exists, INSERT into `db.sessions` with `type='onboarding'`, `domain='industry'`, `status='active'`, `started_at=NOW()`, `last_activity_at=NOW()`, `turns=0`.
4. Set `active_domain = "industry"`.
5. Run the probe-library version migration check (per `solomon-coverage-tracker`): if `coverage.library_version_seen < probe_library/industry.yaml::version` for `domain='industry'`, write a `mentoring_queue` row (`source='probe_library_update'`, `priority=7`).
...
2. For each unfilled `field_id` in declaration order:
   a. Ask the field's `prompt` verbatim from `industry.yaml::required_fields`.
...
2. Run query F3 to render `foundation/00-industry.yaml`. The file format follows the standard header convention used by `foundation/05-non-negotiables.yaml`: a leading comment block, `last_updated`, then a `required_fields` map (one entry per field id with `statement` and `verbatim_phrase`), then a `discovery` map keyed by sub-topic, then a `voice_samples` list (top vocabulary phrases by frequency from this session). DB stays canonical per SOLOMON-PLAN.md §1; the YAML is a derived summary.
3. Print: "Session 0 is complete. Ready to start Session 1?"

## SQL queries

All four use SQLite JSON1 (already a baseline dependency for the keywords arrays).

### F1. Unfilled required_field ids for a session

Inputs: `:session_id`, plus the ordered list of seven `required_field` ids loaded from `industry.yaml`. The skill body iterates the list in declaration order and runs the per-id check below; the returned list of unfilled ids preserves declaration order.

```sql
-- Per-id check. Returns 1 if filled, 0 if unfilled.
...
Pulls every captured row for this session. The `required_field_tag` column extracts the `field:*` value (if any) so the rendering layer can group rows.
...
### F3. Render foundation/00-industry.yaml at session close

Three SQL result sets feed a Python (`pyyaml`) render step.
...
WHERE v.first_seen IN (
    SELECT id FROM captured_items WHERE source_session = :session_id
)
ORDER BY v.frequency DESC, v.last_seen DESC
LIMIT 30;
```
