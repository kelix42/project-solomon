...
**Session 06 (scopes) extends Stage E** with a write to `db.scope_autonomy`. After the standard F3 foundation YAML render, the wrapper iterates each captured `operational_scopes` row (whose `statement` field stores a JSON object `{name, department}`) and runs `INSERT OR IGNORE INTO scope_autonomy (scope, level, since, last_reeval_at, notes) VALUES (?, 0, ?, ?, 'source_session: ' || :session_id)`. This is the only onboarding session that writes to `db.scope_autonomy`. Per §2.11, every scope starts at L0; promotion is performance-driven via Sleep-Cycle Job 7. Owner autonomy preferences per scope are NOT collected at onboarding; they emerge from real usage. Existing scope rows whose level has already been promoted by Job 7 are preserved by `INSERT OR IGNORE`. Department metadata is NOT written to `scope_autonomy` (no column for it); it lives in `captured_items.statement` JSON only.

**Session 06 was retitled** from "taxonomy" to "scopes" on 2026-05-10. The five dropped required_fields (decision_type_taxonomy, product_or_service_categories, vendor_or_supplier_categories, revenue_streams_named, internal_jargon_terms) are captured passively by `solomon-vocabulary-capture`, the listening agent, and corpus entity allowlisting; they no longer earn an onboarding required-field slot.
